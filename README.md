...
- 后台真实登录、会话管理（含登录地）、改密、域名 CRUD、后缀/分类/展示/精品筛选、服务端排序、列显隐、批量管理、分类管理、求购线索、CSV 导入/导出，以及操作日志（筛选/导出/90 天自动清理）；后缀下拉按 D1 实时数量排序，域名 CSV 仅导出域名、注册日期、到期日期、注册商和后缀。
- 自动分类完整对齐 dog.do 的数字、字母、拼音、英文与杂米判断顺序及贪心拼音拆分；自动标签与人工分类在后台分区显示，人工优先且可恢复自动，并保留 `num3…num9`、`pinyin1…pinyin4`、`alpha3/alpha4`、`mixed2/mixed3` 等子类和置信度。
...
- 简介非空：0
- 按 dog.do 口径重新计算：英文 269、拼音 313、字母 118、数字 107、杂米 43、其他 9
