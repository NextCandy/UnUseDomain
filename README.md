...
- 页脚为三栏：左侧友情链接、中间品牌 Logo 与版权（保持视觉居中）、右侧联系方式小图标（原先在页首右上角，已整体下移）。窄屏（≤900px）三栏改为堆叠居中。
...
- 桌面端（`hover: hover`、`pointer: fine` 且 ≥1024px）悬停域名卡片时，域名向四角引出手写体批注，每条写明字段与取值（剩余时间、简介、后缀、注册商），笔画从域名侧起笔、箭头尖落在批注上；悬停期间卡片原有的徽章、简介与到期行压暗到 14%，只留域名与操作按钮清晰。四条按「剩余时间 → 简介 → 后缀 → 注册商」依次浮现，间隔由 `--note-step` 控制，取值为空的批注整条不渲染且不占次序，后面的自动补位。触屏与移动端不加载该效果，`prefers-reduced-motion` 下去掉过渡。样式独立在 `src/client/styles/domain-annotations.css`。
