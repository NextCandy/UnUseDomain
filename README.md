...
- 首页无 Hero：页首（品牌 Logo + 右侧「后台」入口）之下直接是搜索与域名列表；无独立精选区块、无分类 pills 行、无「全部资产」标题行；精品域名以主列表卡片上的精品标记区分。页脚不再有管理入口。
...
- 桌面端（`hover: hover`、`pointer: fine` 且 ≥1024px）悬停域名卡片时浮现手写体字段旁注：后缀、简介、剩余时间带曲线箭头指向对应元素并给该元素加荧光笔底色，注册商没有可指的可视元素因此只显示自带取值的旁注。简介与注册商为空时对应旁注不渲染，避免箭头指向空白；触屏与移动端不加载该效果，`prefers-reduced-motion` 下去掉过渡。样式独立在 `src/client/styles/domain-annotations.css`（前台 CSS 108.66KB → 120.62KB，gzip 19.19 → 20.85KB）。
