...
- 手机列表一行放两条域名条，含后缀超过 15 字符的域名（`DomainCard` 打上 `full-row`）占满整行。靠 `grid-auto-flow: row dense` 让后面的短域名回填长域名跳过的半格，不必在 JS 里重排列表——实测不会留下任何孤零零的半格。半格只有约 110px 放域名，15 字符在 14px 字号下最宽 106px 刚好一行读完，**这个字号是被阈值定死的**；同时要关掉 `strong` 与 `.domain-tld` 各自带的 `white-space: normal`／`overflow-wrap: anywhere`，否则后缀会被甩到第二行。整行的那条仍用 20px 与 34px 按钮。
