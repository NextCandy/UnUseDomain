...
describe("D1 导入计划", () => {
  ...
  it("重新导入 SQL 保护管理员字段，并仅用非空简介更新展示内容", async () => {
    ...
    expect(domainsUpsert).toMatch(/description\s*=\s*CASE WHEN excluded\.description != ''/i);
...
describe("dry-run 字段差异", () => {
  ...
  it("库内 ISO 时间与 CSV 日期只比较日期部分", async () => {
    const record = { ...(await firstRecord()), initialRegisteredAt: "2015-05-12", initialExpiresAt: null, initialRegistrarName: null, initialDescription: "" };
    expect(diffImportRecord(record, {
  ...
  it("CSV 留空的字段不算差异，因为导入会保留原值", async () => {
    const record = { ...(await firstRecord()), initialRegisteredAt: null, initialExpiresAt: null, initialRegistrarName: null, initialDescription: "" };
    expect(diffImportRecord(record, {
      registered_at: "2015-05-12T00:00:00.000Z",
      expires_at: null,
  ...
  it("列出会被改写字段的新旧值", async () => {
    const record = { ...(await firstRecord()), initialRegisteredAt: null, initialExpiresAt: null, initialRegistrarName: "Spaceship", initialDescription: "" };
    expect(diffImportRecord(record, {
      registered_at: null,
      expires_at: null,
      registrar_name: "易名",
